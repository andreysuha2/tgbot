...
@router.callback_query(AcсeptModeratorCallbackFactory.filter(F.action == "accept"))
async def accept_modarator_callback(callback: types.CallbackQuery, callback_data: AcсeptModeratorCallbackFactory):
    await user_controller.accept_moderator(callback, callback_data.user_id)

@router.callback_query(AcсeptModeratorCallbackFactory.filter(F.action == "decline"))
async def decline_moderator_callback(callback: types.CallbackQuery, callback_data: AcсeptModeratorCallbackFactory):
